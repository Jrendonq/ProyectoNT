...
def filtro3():
    ...
    with col2:
        momento = st.selectbox("Momento", momentosU)
    resultado = df[(df['JORNADA'] == jornada) & (df['MOMENTO'] == momento)]
   
    resultado = resultado.reset_index(drop=True) 
    # Grafico de torta
    labels = ['CONOCIMIENTO', 'DESEMPEÑO', 'PRODUCTO']
    valores = [resultado['CONOCIMIENTO'].mean(), resultado['DESEMPEÑO'].mean(), resultado['PRODUCTO'].mean()]
